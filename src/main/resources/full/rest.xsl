	...
	<xsl:template match="jpa:entity[j:process(@class, $include, $exclude)]">
		...
		<xsl:if test="j:generate('REST', current())">
			<x:file name="{$name}Rest.java" dir="{j:packagePath($packageBase)}" layer="rest">
				...
				import <xsl:value-of select="$packageLib"/>.web.HTTPStatic;
				import <xsl:value-of select="$packageLib"/>.web.RuntimeStatic;
				...
				HTTPStatic.info("Se ha actualizado el registro: " + value);
				return value;
				}
				@POST
				@Path("delete")
				...
				HTTPStatic.info("Se ha eliminado el registro: " + value);
				return value;
				}
				}
			</x:file>
